...
CREATE OR REPLACE SCHEMA SNOWFLAKE_CODE_SAMPLE.TPCH WITH MANAGED ACCESS
...
BEGIN TRANSACTION;
    CREATE OR REPLACE TABLE SNOWFLAKE_CODE_SAMPLE.TPCH.PARTSUPP AS
    SELECT PS_PARTKEY,
    ...
    CREATE OR REPLACE TABLE SNOWFLAKE_CODE_SAMPLE.TPCH.SUPPLIER AS
...
--SHOW TABLES LIKE 'PARTSUPP' IN SCHEMA SNOWFLAKE_CODE_SAMPLE.TPCH;
--SHOW TABLES LIKE 'SUPPLIER' IN SCHEMA SNOWFLAKE_CODE_SAMPLE.TPCH;
SHOW TABLES IN SCHEMA SNOWFLAKE_CODE_SAMPLE.TPCH;

-- 4.- Exploring the tables

-- PARTSUPP
SELECT * FROM SNOWFLAKE_CODE_SAMPLE.TPCH.PARTSUPP LIMIT 5;

-- SUPPLIER
SELECT * FROM SNOWFLAKE_CODE_SAMPLE.TPCH.SUPPLIER LIMIT 5;
...
ALTER WAREHOUSE SCS SUSPEND;

ALTER SESSION SET QUERY_TAG = 'Query 1: Without SO';

SELECT *
FROM SNOWFLAKE_CODE_SAMPLE.TPCH.PARTSUPP PS
INNER JOIN SNOWFLAKE_CODE_SAMPLE.TPCH.SUPPLIER S
...
SELECT APPROX_COUNT_DISTINCT(PS_PARTKEY) FROM SNOWFLAKE_CODE_SAMPLE.TPCH.PARTSUPP;
SELECT APPROX_COUNT_DISTINCT(S_NATIONKEY) FROM SNOWFLAKE_CODE_SAMPLE.TPCH.SUPPLIER;

-- Adding Search Optimization
ALTER TABLE SNOWFLAKE_CODE_SAMPLE.TPCH.PARTSUPP ADD SEARCH OPTIMIZATION ON EQUALITY(PS_SUPPKEY);
ALTER TABLE SNOWFLAKE_CODE_SAMPLE.TPCH.SUPPLIER ADD SEARCH OPTIMIZATION ON EQUALITY(S_NAME, S_NATIONKEY), SUBSTRING(S_NAME);

-- Now, let's verify that Search Optimization is enabled and the backend process has finished indexing our data
DESCRIBE SEARCH OPTIMIZATION ON SNOWFLAKE_CODE_SAMPLE.TPCH.PARTSUPP;
DESCRIBE SEARCH OPTIMIZATION ON SNOWFLAKE_CODE_SAMPLE.TPCH.SUPPLIER;
...
INNER JOIN SNOWFLAKE_CODE_SAMPLE.TPCH.SUPPLIER S
 ON PS.PS_SUPPKEY = S.S_SUPPKEY
WHERE PS.PS_SUPPKEY ='9999990'
 AND S.S_NATIONKEY NOT LIKE '2';
ALTER SESSION UNSET QUERY_TAG;

-- Cost of a enabled search optimization on the table SUPPLIER for a equality predicate on the column S_SUPPKEY
SELECT SYSTEM$ESTIMATE_SEARCH_OPTIMIZATION_COSTS('SNOWFLAKE_CODE_SAMPLE.TPCH.SUPPLIER', 'EQUALITY(S_SUPPKEY)')
