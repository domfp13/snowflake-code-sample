...
-- ****************************** 2.- Internal Stage ******************************
CREATE OR REPLACE STAGE SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_INTERNAL_ACCOUNTS
    DIRECTORY = ( ENABLE = TRUE );

-- LOADING LOCAL FILE USING CLIENT OR UI
PUT file://///Users/eplata/Developer/personal/snowflake-sample-code/sql/03-loading/SNOWBANK_PUBLIC_ACCOUNTS_1.json @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_INTERNAL_ACCOUNTS;

-- SELECT DATA FROM THE STAGE, HERE THE DATA HAS NOT BEEN LOADED YET INTO A TABLE.
SELECT *, metadata$filename, metadata$file_row_number FROM @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_INTERNAL_ACCOUNTS (FILE_FORMAT => SNOWFLAKE_CODE_SAMPLE.PUBLIC.FILE_FORMAT_JSON_GENERIC) SAMPLE(100 ROWS);

-- LIST OR LS COMMAND TO SEE THE FILES IN THE STAGE
LIST @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_INTERNAL_ACCOUNTS;

-- CREATING TABLE TO LOAD THE DATA FROM THE STAGE
CREATE OR REPLACE TRANSIENT TABLE SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_INTERNAL_RAW (
    PAYLOAD VARIANT,
    LOAD_DATE TIMESTAMP_LTZ(9) DEFAULT CURRENT_TIMESTAMP()
);

-- LOADING LOCAL FILE USING COPY INTO COMMAND
COPY INTO SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_INTERNAL_RAW (PAYLOAD)
FROM (
    SELECT $1 AS PAYLOAD
    FROM @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_INTERNAL_ACCOUNTS (FILE_FORMAT => SNOWFLAKE_CODE_SAMPLE.PUBLIC.FILE_FORMAT_JSON_GENERIC)
)
...
SELECT * FROM SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_INTERNAL_RAW;

SELECT COUNT(*) FROM SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_INTERNAL_RAW;

-- LOADING LOCAL FILE USING CLIENT
PUT file://///Users/eplata/Developer/personal/snowflake-sample-code/sql/03-loading/SNOWBANK_PUBLIC_ACCOUNTS_2.json @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_INTERNAL_ACCOUNTS;

LIST @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_INTERNAL_ACCOUNTS;

REMOVE @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_INTERNAL_ACCOUNTS/SNOWBANK_PUBLIC_ACCOUNTS_2.json.gz;
...
CREATE OR REPLACE STAGE SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_EXTERNAL_DEPOSITORS
...
SELECT * FROM @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_EXTERNAL_ACCOUNTS/SNOWBANK_PUBLIC_ACCOUNTS_1.json LIMIT 10;
...
FROM @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_EXTERNAL_ACCOUNTS SAMPLE(100 ROWS);

-- DEPOSITOR DATA
SELECT * FROM @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_EXTERNAL_DEPOSITORS;
...
FROM @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_EXTERNAL_DEPOSITORS SAMPLE (100 ROWS);

-- ****************************** 4.- Creating Table ******************************
-- COPY INTO APPROACH: ACCOUNTS DATASET
CREATE OR REPLACE TABLE SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_RAW (
...
SELECT * FROM SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_RAW;

COPY INTO SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_RAW (ACCESSIBLE_BALANCE,ACCOUNT_BALANCE,ACCOUNT_STATUS_CODE,
...
SELECT TOP 10 * FROM SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_RAW;

-- CTAS APPROACH instead of COPY INTO
DROP TABLE IF EXISTS SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_RAW;

CREATE OR REPLACE TABLE SNOWFLAKE_CODE_SAMPLE.PUBLIC.ACCOUNTS_RAW AS
SELECT $1:ACCESSIBLE_BALANCE::VARCHAR AS ACCESSIBLE_BALANCE,
       $1:ACCOUNT_BALANCE::VARCHAR AS ACCOUNT_BALANCE,
       $1:ACCOUNT_STATUS_CODE::VARCHAR AS ACCOUNT_STATUS_CODE,
       $1:ACCOUNT_UID::VARCHAR AS ACCOUNT_UID,
       ...
       metadata$filename::VARCHAR AS FILE_NAME,
       metadata$file_row_number::VARCHAR AS FILE_ROW_NUMBER
FROM @SNOWFLAKE_CODE_SAMPLE.PUBLIC.STAGE_EXTERNAL_ACCOUNTS (FILE_FORMAT => SNOWFLAKE_CODE_SAMPLE.PUBLIC.FILE_FORMAT_JSON_GENERIC);
